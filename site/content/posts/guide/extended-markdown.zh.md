---
title: "扩展 Markdown：公式、图表等"
date: "2026-08-07"
updated: "2026-08-11"
author: "Jane Doe"
tags: ["mdweb", "markdown", "math"]
meta:
  description: "mdweb 原生渲染的全部扩展语法一览——无 JavaScript、无第三方依赖。"
---

这里展示 mdweb 的 Markdown 引擎在 CommonMark 之外支持的全部语法：
行内公式、块级公式、绘图环境、Mermaid 图表、Graphviz 与 PlantUML、
表格、警告框、定义列表、上标/下标、高亮、Emoji 和 HTML 注释——全部在
构建时编译为内联 SVG 或带样式的 HTML，无需任何客户端库。

[[TOC title="目录"]]

## 行内与块级公式

行内公式写作 `$a^2 + b^2 = c^2$`（或等价的 `$$…$$` / `\(…\)`），以
浏览器原生 MathML 渲染——无需任何库或 JavaScript。块级公式写在
` ```math ` 围栏中，也可使用 LaTeX 的 `\[…\]` 形式或多行
`$$\n…\n$$` 块：

```math
\sum_{i=1}^{n} i = \frac{n(n+1)}{2}
```

分式、根号与希腊字母都支持：$E = mc^2$、$\sqrt{x}$、
$\alpha\beta\gamma$。行内求根公式：

> 一元二次方程 $ax^2 + bx + c = 0$ 的求根公式为
> $x = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}$，沿用至今。

关于积分、矩阵、xypic 与 TikZ 的更多示例，见
[数学示例](/pages/docs/math-examples/) 页面。

## 绘图环境

除公式外，语言标记为 `latex` / `tikz` / `xy` / `picture` 的围栏代码块
（或 `\[…\]` 包裹）还接受 `picture`、xy-pic 与 TikZ 三种经典绘图包。
标签里出现的公式仍由同一个数学引擎排版：

```math
\xymatrix{
  A \ar[r]^f \ar[d]_g & B \ar[d]^{g'} \\
  D \ar[r]_{f'}        & C
}
```

带坐标轴、正弦曲线与标签的 TikZ 绘图：

```latex
\begin{tikzpicture}
\draw[->] (-3.5,0) -- (3.5,0) node[right] {$x$};
\draw[->] (0,-1.5) -- (0,1.5) node[above] {$y$};
\draw[color=blue] plot[domain=-3.1416:3.1416,samples=100] (\x,{sin(\x r)}) node[above right] {$y=\sin x$};
\end{tikzpicture}
```

## Mermaid 流程图

标记为 `mermaid` 的围栏代码块渲染为内联 SVG。引擎支持
`graph` / `flowchart` 头、四种方向（`TD`/`LR`/`RL`/`BT`）、七种节点
形状、四种边样式（`-->`、`---`、`-.->`、`==>`）、`|label|` 与
`-- label -->` 边标签、`subgraph` 子图与 `%%` 注释：

```mermaid
flowchart TD
    A[编写 Markdown] --> B{能渲染了吗？}
    B -- 可以 --> C[内联 SVG]
    B -- 不行 --> D[调试]
    C --> E[发布]
```

## 更多原生图表

`pie` 饼图与 `gantt` 甘特图原生渲染，Graphviz `digraph` 与 PlantUML
类图同样如此。举例如下：

```mermaid
pie title 编程语言占比
    "Python" : 45
    "JavaScript" : 30
    "Go" : 25
```

```dot
digraph G {
  A -> B -> C;
  B -> D;
}
```

mdweb 原生不认识的语法（如 Mermaid 的 `sequenceDiagram`、
`stateDiagram-v2`、`journey`，PlantUML 的 `sequence` / `activity` 等）
会自动退化为代码块，调试时不会破坏页面。

## Markdown 表格

GFM 管道表格与其它渲染器一致——无需插件、无需额外围栏：

| 月份   | 销售额（万） | 同比增长 |
| ------ | ------------ | -------- |
| 1 月   | 120          | +5%      |
| 2 月   | 135          | +12%     |
| 3 月   | 98           | −3%      |

## 警告框

GitHub 风格的提示会渲染为带样式的盒子。提示类型为 `[!NOTE]` /
`[!TIP]` / `[!IMPORTANT]` / `[!WARNING]` / `[!CAUTION]`（另支持 `[!INFO]`、
`[!SUCCESS]`、`[!DANGER]`）；自定义标题可写在标记后的同一行：

> [!NOTE]
> 供用户了解的有用信息，即使快速浏览也应知晓。

> [!TIP] 好的建议
> 帮助你成功的更好的建议。

> [!IMPORTANT]
> 用户达成目标所需的关键信息。

> [!WARNING]
> 需要立即注意以避免问题的紧急信息。

> [!CAUTION]
> 提醒潜在风险或不良后果。

> [!INFO]
> 非关键的背景补充信息。

> [!SUCCESS]
> 确认操作成功或达成预期结果。

> [!DANGER]
> 留给系统级、可导致崩溃的严重后果。

## 定义列表

段落后跟 `:` 行即成为定义列表：

术语
: 该术语的含义，用一行说明
: 同一术语的第二个释义

## 上标、下标与高亮

化学式：H~2~O 与 x^2^；用 ==双等号== 高亮重要短语。

## Emoji

短代码展开为 Emoji：:rocket: :smile: :wave: —— 未知代码（如 `:no_such_code:`）
保持原样。

## HTML 注释

注释 `<!-- 就像这样 -->` 会从输出中完全移除，可以在草稿里留下私人备忘。

## 目录标签 `[[TOC]]`

把 `[[TOC]]` 单独写在一行，就能自动生成一份目录——它会被替换成
`<nav class="toc">` 包裹的 `<ul>` 列表，列出文档里所有的标题。
可以放在正文开头（页面顶部目录）或任意位置（章节末尾回顾）。
同一个文档里可以出现多次 `[[TOC]]`，每一处都会独立渲染。

支持的写法：

- `[[TOC]]` — 默认形式，列出全部标题
- `[TOC]` — 单层方括号也可以
- `[[toc]]` / `[[Toc]]` — 大小写不敏感
- `[[TOC max-depth=3]]` — 只保留 H1–H3，H4 及更深的不出现
- `[[TOC title="目录"]]` — 在目录上方加一行标题
- `[[TOC max-depth=2 title="本页内容"]]` — 两个属性任意顺序组合
- 未知属性会被安静忽略，不会让整个标签失效

示例：

```markdown
[[TOC title="目录"]]

## 第一章
### 1.1 节
### 1.2 节
## 第二章
```

会渲染为：

```html
<p class="toc-title">目录</p>
<nav class="toc"><ul>
<li><a href="#第一章">第一章</a><ul>
<li><a href="#11-节">1.1 节</a></li>
<li><a href="#12-节">1.2 节</a></li>
</ul></li>
<li><a href="#第二章">第二章</a></li>
</ul></nav>
```

注意：标题锚点由 `slugify()` 生成——大写转小写、连续非字母数字
字符折叠为单个 `-`，其余字符原样保留。这意味着 CJK / Cyrillic /
带音标的拉丁字符都会留在 ID 里（HTML5 允许任意字符作为 `id`，
浏览器在 URL fragment 里自动百分号编码）。点击目录里指向
`#行内与块级公式` 的链接时浏览器会跳到对应 `<h2 id="行内与块级公式">`
的位置。